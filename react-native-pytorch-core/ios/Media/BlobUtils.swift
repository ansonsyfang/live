...
public class BlobUtils : NSObject {
  ...
  @objc
  public static func torchlive_media_beginReadData(cRefId: UnsafePointer<CChar>) -> UnsafePointer<CChar>? {
  ...
  @objc
  public static func torchlive_media_endReadData(cRefId: UnsafePointer<CChar>) {
    let refId = String(cString: cRefId)
    MediaToBlobCache.end(idRef: refId)
  }
  
  @objc
  public static func torchlive_media_getDirectBytes(cRefId: UnsafePointer<CChar>) -> UnsafeMutablePointer<UInt8>? {
    let refId = String(cString: cRefId)
  ...
  @objc
  public static func torchlive_media_getDirectSize(cRefId: UnsafePointer<CChar>) -> size_t {
    let refId = String(cString: cRefId)
    let mediaData = MediaToBlobCache.get(idRef: refId)
